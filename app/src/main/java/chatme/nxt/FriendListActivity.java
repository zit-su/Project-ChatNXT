...
public class FriendListActivity extends AppCompatActivity {
	...
	public class Recyclerview1Adapter extends RecyclerView.Adapter<Recyclerview1Adapter.ViewHolder> {
		...
		public void onBindViewHolder(ViewHolder _holder, final int _position) {
			...
			}.getIns((int)20, 0xFF444444));
			
			if (_data.get((int)_position).containsKey("id")) {
				
				final String user2 = _data.get((int)_position).get("id").toString();
				final String myUid = FirebaseAuth.getInstance().getCurrentUser().getUid();
				final boolean isMe = user2.equals(myUid);
				...
				if (_data.get((int)_position).containsKey("username")) {
					if (isMe) {
						textview1.setText("Chat Yourself");
					} else {
				...
				if (_data.get((int)_position).containsKey("dp")) {
					...
					.load(Uri.parse(_data.get((int)_position).get("dp").toString()))
				...
				if (isMe) {
					// No friend request check for yourself
				} else if (_data.get((int)_position).containsKey("type")
				&& "group".equals(_data.get((int)_position).get("type").toString())) {
					...
					inboxRef.child(myUid).child(user2).addListenerForSingleValueEvent(new ValueEventListener() {
						@Override
						public void onDataChange(@NonNull DataSnapshot inboxSnapshot) {
							...
							if (inboxSnapshot.exists()) {
								...
								friendRequestRef.child(user2).child(myUid)
				...
				linear1.setOnClickListener(new View.OnClickListener() {
					...
					public void onClick(View _view) {
						...
						if (isMe) {
							ax.setClass(getApplicationContext(), ChatActivity.class);
							ax.putExtra("user2", myUid);
							startActivity(ax);
							return;
						}
						...
						if (_data.get((int)_position).containsKey("type")) {
							...
							_data.get((int)_position).get("user").toString().concat(" : Not updated! ")
							);
						}
					}
				});
